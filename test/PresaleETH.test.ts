...
describe("presaleETH", function () {
  ...
  beforeEach(async () => {
    ...
    const PresaleETH = await ethers.getContractFactory("MockPresaleETH");
    startTime = Date.parse('21 Jun 2022 00:12:00 GMT') / 1000;

    endTime = startTime + 30 * 24 * 3600;
    presaleETH = (await PresaleETH.deploy(inkToken.address, startTime, endTime)) as MockPresaleETH;
    await presaleETH.deployed();

    await inkToken.mint(presaleETH.address, BigNumber.from("43750000000"));
    await inkToken.mint(owner.address, BigNumber.from("10000"));
    ...
    await inkToken.connect(owner).approve(presaleETH.address, 10000);
  ...
  it("test_initialization", async function () {
    expect(await inkToken.balanceOf(presaleETH.address)).to.equal(BigNumber.from("43750000000"));
  ...
  it("test_setRange_asUser_thenReverted", async () => {
    await expect(presaleETH.connect(user1).setRange(10, 100)).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("test_setRange_asOwner_givenInValidRange_thenReverted", async () => {
    await expect(presaleETH.connect(owner).setRange(100, 10)).to.be.revertedWith("set invalid range.");

    await expect(presaleETH.connect(owner).setRange(0, 100)).to.be.revertedWith("set invalid range.");
  });

  it("test_setRange_asOwner_givenValidRange_thenSuccess", async () => {
    await presaleETH.connect(owner);
    await presaleETH.setRange(10, 100);

    expect(await presaleETH.minAmount()).to.equal(10);
    expect(await presaleETH.maxAmount()).to.equal(100);
  });
  ...
  it("test_invest_givenLessThanMinAmount_thenReverted", async () => {
    await presaleETH.connect(owner);
    await presaleETH.setRange(ethers.utils.parseEther("10"), ethers.utils.parseEther("100"));
  ...
  it("test_invest_givenValidAmount_thenSuccessAndEmitFundsTransferEvent", async () => {
    ...
    await expect(await presaleETH.checkFundsETH(user1.address)).to.equal(ethers.utils.parseEther("20"));
    await expect(await presaleETH.connect(owner).amountRaisedETH()).to.equal(ethers.utils.parseEther("20"));
    await expect(await presaleETH.checkFundsINK(user1.address)).to.equal(20 * 100000000);
    await expect(await presaleETH.connect(owner).amountRaisedINK()).to.equal(20 * 100000000);
  ...
  it("test_invest_reachGoal_thenSuccessAndEmitGoalReachedEvent", async () => {
    await presaleETH.connect(owner);
    await presaleETH.setRange(ethers.utils.parseEther("10"), ethers.utils.parseEther("10000"));

    await presaleETH.connect(owner).setPrice(100000000);
    const tx = await presaleETH.connect(user1).invest({ value: ethers.utils.parseEther("4376") });

    await expect(await presaleETH.checkFundsETH(user1.address)).to.equal(ethers.utils.parseEther("4376"));
    await expect(await presaleETH.connect(owner).amountRaisedETH()).to.equal(ethers.utils.parseEther("4376"));
    await expect(await presaleETH.checkFundsINK(user1.address)).to.equal(4376 * 100000000);
    await expect(await presaleETH.connect(owner).amountRaisedINK()).to.equal(4376 * 100000000);
    ...
    await expect(tx).to.emit(presaleETH, 'GoalReached').withArgs(user1.address, 4376 * 100000000);
  });

  it("test_getClamiableAmount_beforePresaleClosed_thenReverted", async () => {
    await expect(await presaleETH.presaleClosed()).to.be.false;
    const now = await presaleETH.getTimestamp();
  ...
  it("test_getClamiableAmount_zeroAmountPaid_thenReverted", async () => {
    ...
    await expect(presaleETH.connect(user2).getClaimableAmount()).to.be.revertedWith("zero amount paid.");
    await ethers.provider.send('evm_revert', [snapShot]);
  });
  ...
  it("test_getClamiableAmount_beforeCliff_thenReverted", async () => {
    ...
    await expect(presaleETH.connect(user2).getClaimableAmount()).to.be.revertedWith("claim is not available before cliff.");
    await ethers.provider.send('evm_revert', [snapShot]);
  });
  ...
  it("test_claim_afterPresaleClosed_thenSuccess", async () => {
    ...
    await expect(await presaleETH.checkFundsINK(user1.address)).to.equal(0);
    await ethers.provider.send('evm_revert', [snapShot]);
  });
  ...
  it("test_withdrawinkToken_asOwner_afterClosed_thenSuccess", async () => {
    ...
    await expect((await inkToken.balanceOf(presaleETH.address)).toNumber()).to.equal(2000000000);
    await expect((await inkToken.balanceOf(owner.address)).toNumber()).to.equal(41750010000);
  ...
  it("test_deposit_asOwner_givenNonZeroAmount_thenSuccess", async () => {
    await presaleETH.deposit(3000);
    await expect((await inkToken.balanceOf(owner.address)).toNumber()).to.equal(7000);
    await expect((await inkToken.balanceOf(presaleETH.address)).toNumber()).to.equal(43750003000);
  })
});
